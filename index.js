...
app.post('/Search', async (req, res) => {
    ...
    try {
        ...
        const {title, subtitle, cover: { large: bookcover },} = record;
        ISBN_S.push(ISBN);
        bookd_name.push(title);
        bookd_title.push(subtitle);
        bookd_cover.push(bookcover);
        bookd_publishDates.push(publishDates);
        res.render('partials/addbooks.ejs', { bookData: { title, subtitle, bookcover, publishDates, ISBN} });
    } catch (error) {
